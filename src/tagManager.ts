...
import { insertAutoCloseTag } from "./autoClose";
...
export class TagManager {
  ...
  private getCurrentWordForNewActiveTextEditor(
    ...
    const word = this.getWordAtPosition(document, selection.active);
    this._word = word;
  }
    ...
    while ((result = regex.exec(text)) !== null) {
      if (!result[1]) {
        ...
        if (
          ...
          character <= result.index + 1 + result[1].length
        ) {
          return result[1];
        }
      }
  ...
  private findAndReplacePairedTag(
    document: vscode.TextDocument,
    editor: vscode.TextEditor | undefined,
    cursorPositon: vscode.Position,
    newTag: Tag
    ...
    editor!.edit(
      editBuilder => {
        editBuilder.replace(
          new vscode.Range(
            document.positionAt(pairedTag!.startOffset),
            document.positionAt(pairedTag!.endOffset)
          ),
          newTag.word
        );
      },
